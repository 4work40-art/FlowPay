...
const {
  recognize, classifyDocument, findDate, findNumber, findAmountKopecks, findVat,
  findInns, findKpp, findKpps, findOgrns, findBankRequisites,
  findSupplierName, findSupplierNameFallback, findBuyerName, findPurpose, findReferencedInvoiceNumber,
  findPriority, findUin, findKbk, findOktmo, splitPayerPayee, confidenceFor,
  flattenRowsToText, findStructuredAmount,
...
// Регресс на реальный присланный документ: счёт «СЧЕТ № 00085 от
// 28.05.2026» — без слов «на оплату» — с блоком банковских реквизитов
// получателя перед номером счёта. Раньше классификатор путал такой
// документ с платёжным поручением из-за метки «Банк получателя».
test('classifyDocument: "СЧЕТ № N от ДД.ММ.ГГГГ" без "на оплату" — всё равно invoice, не платёжка', () => {
  const text = [
    'АО "БАНК", г Москва',
    'Банк получателя                    БИК 044525593',
    'ИНН 7724239974 КПП 772401001',
    'СЧЕТ № 00085 от 28.05.2026',
    'Итого к оплате: сто рублей',
  ].join('\n');
  assert.strictEqual(classifyDocument(text), 'invoice');
});

// Регресс: шаблоны без явной метки "Поставщик:"/"Продавец:" — например,
// когда метка "Получатель" стоит отдельной строкой от названия компании
// (значение находится в другой ячейке/строке, не в этой же строке —
// обычная regex-подстановка после метки тут ничего не находит), либо
// название компании просто напечатано в шапке документа без всякой метки.
test('findSupplierNameFallback: первая строка, похожая на название организации', () => {
  const text = [
    'СЧЕТ № 1 от 01.01.2026',
    'ООО «Ромашка»',
    'ИНН 7707083893',
  ].join('\n');
  assert.strictEqual(findSupplierName(text), null); // явной метки нет
  assert.strictEqual(findSupplierNameFallback(text), 'ООО «Ромашка»');
});

test('findStructuredAmount: сумма из ячейки рядом с меткой "Итого:" без десятичных знаков', () => {
  const rows = [
    ['Наименование', 'Цена', 'Кол-во', 'Сумма'],
    ['Товар', 100, 2, 200],
    [null, null, 'Итого:', 200],
    [null, null, 'В том числе НДС:', 0],
  ];
  assert.strictEqual(findStructuredAmount(rows, /^итого\s*:?$/i), 20000); // в копейках
  assert.strictEqual(findStructuredAmount(rows, /в\s*том\s*числе\s*ндс/i), null); // 0 не считается найденным без allowZero
  assert.strictEqual(findStructuredAmount(rows, /в\s*том\s*числе\s*ндс/i, true), 0);
  assert.strictEqual(findStructuredAmount(null, /итого/i), null);
});

test('flattenRowsToText: строки ячеек -> текст построчно, даты как ISO', () => {
  const rows = [['A', null, 'B'], [], ['C']];
  assert.strictEqual(flattenRowsToText(rows), 'A B\nC');
});

// Полный сквозной регресс-тест на структуру реального документа, который
// сломал распознавание: счёт с товарной таблицей (числа в ячейках без
// форматирования ",00"), название поставщика без метки, но напечатанное
// первой строкой, и метка "Получатель" в блоке банковских реквизитов
// отдельно от названия организации. Файл собран через ExcelJS (не привязан
// к внешним бинарникам, детерминирован в CI) и прогоняется через recognize()
// целиком — так же, как обрабатывался бы настоящий .xlsx/.xls от пользователя.
test('recognize: Excel-счёт с товарной таблицей и меткой "Получатель" распознаётся полностью', async () => {
  const wb = new ExcelJS.Workbook();
  const sheet = wb.addWorksheet('Sheet1');
  [
    ['ООО "Ромашка"'],
    ['Банк получателя', null, null, null, 'БИК', '044525593'],
    ['ИНН 7724239974 КПП 772401001'],
    ['СЧЕТ № 00085 от 28.05.2026'],
    [],
    ['№', 'Наименование товара', 'Цена', 'Кол-во', 'Сумма'],
    [1, 'Дверной упор', 87, 480, 41760],
    [2, 'Ограничитель', 200, 30, 6000],
    [null, null, null, 'Итого:', 47760],
    [null, null, null, 'В том числе НДС:', 0],
  ].forEach(row => sheet.addRow(row));
  const buf = await wb.xlsx.writeBuffer();

  const result = await recognize(buf, 'invoice.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  assert.strictEqual(result.doc_type, 'invoice');
  assert.strictEqual(result.fields.number, '00085');
  assert.strictEqual(result.fields.invoice_date, '2026-05-28');
  assert.strictEqual(result.fields.amount_kopecks, 4776000);
  assert.strictEqual(result.fields.supplier_name, 'ООО "Ромашка"');
