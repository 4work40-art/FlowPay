...
- Умная загрузка документов: перетащите PDF/JPG/PNG счёта на оплату, счёта-
  фактуры/УПД или платёжного поручения — поля формы заполняются
  автоматически по полному набору реквизитов из законодательства РФ
  (номер, дата, сумма, ставка/сумма НДС, ИНН/КПП/ОГРН, банковские
  реквизиты плательщика и получателя, очередность платежа, УИН/КБК/ОКТМО).
  Платёжка сама находит нужный счёт по номеру в назначении. Текстовые PDF
  читаются напрямую, сканы и фото — через OCR (Tesseract, рус+англ) с
  предобработкой изображения. Файл нигде не сохраняется дольше одного
  запроса. Импорт реестра счетов из Excel/CSV (`/invoices/import`) —
  разбор таблицы, проверка каждой строки и пакетное создание счетов.
  Всегда требует проверки перед сохранением.
- Контрагенты: карточки с ИНН/КПП/ОГРН/банковскими реквизитами, автоматический подсчёт долга по каждому
