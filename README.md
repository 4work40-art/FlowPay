...
- Умная загрузка документов: перетащите PDF/JPG/PNG счёта на оплату или
  платёжного поручения — поля формы (номер, сумма, дата, ИНН/КПП, назначение
  платежа) заполняются автоматически, платёжка сама находит нужный счёт по
  номеру в назначении. Текстовые PDF читаются напрямую, сканы и фото — через
  OCR (Tesseract, рус+англ). Всегда требует проверки перед сохранением.
